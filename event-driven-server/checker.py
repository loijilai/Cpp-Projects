...
class TelnetClient(telnetlib.Telnet):
    ...
    def __init__(self, port, server_type = 0): # 0: read, 1: write
        self.log = ""
        self.response = ""
    ...
    def handle_EOF(self, inputs, check):
        ...
        condition1 = (check or self.read_all(add_to_log=True) == 0)
        condition2 = self.check_connection() == 2
        condition3 = self.checker.check_invalid(self.log, idx=-1, delimeter=": ")
        if not condition1:
